                ...
                {categories.map((category, index) => (  
                    <Link className="rounded-full" key={index} href={`/${category.slug}`}>                 
                        <div  className='relative rounded-full w-full group overflow-hidden'>
                            ...
                            <p className='cursor-pointer absolute xl:group-hover:top-32 lg:group-hover:top-12 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#EEEDE7] text-black text-xl max-xl:text-sm max-md:text-xl rounded-3xl max-xl:px-3 max-md:px-8 w-[60%] text-center py-1 duration-500'>
                                {category.name}
                            </p>
